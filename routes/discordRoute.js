...
Router.get('/permission',async(req,res)=>{
    ...
    try {
        ...
        const guilds=getArray(client.guilds.cache);
        for (let i = 0; i < guilds.length; i++) {
            const g = guilds[i];
            const data=await GuildModel.findOne({guildID:g.id});
            if(data)
            {
                const role=await g.roles.fetch(data.roleID);
                const member=await g.members.fetch(did)
                if(member)
                {
                    const roleArray=getArray(member.roles.cache)
                    const validRoles=roleArray.filter(r=>r.id===role.id);
